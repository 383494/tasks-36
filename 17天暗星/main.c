...
int randbetween(int start, int end){
	return rand()/(float)RAND_MAX*end;
}
...
int main(int argc, char *argv[])
{
	...
	SDL_Point pts[4];//pts[0]为p

	for(int i=0;i<4;i++){
		pts[i].x = randbetween(0,640);
		pts[i].y = randbetween(0,480);
	}
	int n = 0;
	SDL_SetRenderDrawColor(ren,255,255,255,255);
	for(int i=0;i<30000;i++){
		SDL_RenderDrawPoints(ren, pts, 1);
		n = randbetween(1,3);
		pts->x = (pts->x + pts[n].x)/2;
		pts->y = (pts->y + pts[n].y)/2;
	}
